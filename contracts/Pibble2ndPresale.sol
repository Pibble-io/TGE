...
contract Crowdsale {
  ...
  function _forwardFundsWei(uint256 txAmount) internal {
    ...
    //msg.value = msg.value.sub(msg.value.sub(txAmount));
    //uint256 initVal = msg.value;
    //wallet.transfer(msg.value.sub(initVal.sub(txAmount)));

    wallet.transfer(txAmount);

/**
    if(msg.value==txAmount){
 ...
 */
contract Pibble2ndPresale is Crowdsale, Ownable {
 ...
 *  
 */
  function Pibble2ndPresale(uint256 _openingTime, uint256 _closingTime, uint256 _rate, address _wallet, uint256 _cap, MintableToken _token, uint256 _minValue, uint256 _maxValue) public
...
contract PibbleToken is MintableToken {
  ...
  string public constant name = "Pibble Crowdsale Token"; // solium-disable-line uppercase
